            ...
            <div class="card">
                ...
                <div class="card-body" style="padding:30px">
                    ...
                    <form  method="POST">
                        ...
                        @if(Auth::user()->roles->contains(1) && !$user->roles->contains(1))
                            <div class="form-group">
                                <label for="rol">Rol</label>
                                <select name="rol" id="rol" class="form-control">
                                    @foreach($roles as $r => $rol)
                                        <option value="{{$rol->name}}"  @if($user->roles->contains($rol->id)) selected @endif  >{{$rol->description}}</option>
                                    @endforeach
                                </select>
                            </div>
                        @endif
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="ciudad" id="ciudad" class="form-control" value="{{$user->ciudad}}">
                        </div>
                        @error('ciudad')
                        <p style="color:red">{{$message}}</p>
                        @enderror
